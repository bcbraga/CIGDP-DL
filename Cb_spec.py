"""
Este código é uma implementação feita por Bruna Cristina Braga Charytitsch do método GRASP_SPEC: Cb_spec + Local Search.
O método para construção de uma solução inicial G_SPEC utilizando a técnica SPECTRAL para geração de embeddings foi
...
This code is an implementation by Bruna Cristina Braga Charytitsch of the GRASP_SPEC: Cb_spec + Local Search method.
The method for constructing an initial solution G_SPEC using the SPECTRAL technique for generating embeddings was
...
def pre_emb_data(graph0, nl0, nel0):
    ...
    # specify the dimension as the number of nodes minus one (dimension = number of nodes - 1), this will ensure that
    # the value of k is smaller than the dimensions of matrix A. The error occurs when k is greater than or equal to
    # the number of columns or rows of the matrix, so setting k in this way should help avoid this problem
    # (because SDV decomposition).

    # model
    model = Spectral(g_cogdl.num_nodes - 1)
...
def swap_func(grafos8, nl8, nel8, p8, t_cro, ini_cro):
    ...
    while improvement:
        ...
        for i in np.arange(nl8):
            ...
            no_inc = np.array(p8[aux + np.msort(ord_inc)])
...
def insertion_func(grafos_if, nl_if, nel_if, cro_if, p_i0, k_if, ini_cr):
    ...
    while improvement:
        ...
        for io in np.arange(nl_if):
            ...
            # lista com nos incrementais na ordem em que aparecem no layer
            # List of incremental nodes in the order they appear in the layer.
            nos_incr = np.array(p_i0[aux + np.msort(ord_inc)])
...
def main():
    ...
    for ru in np.arange(total_run):
        ...
        arq_sp = open('res_spec/Cb_spec_' + str(ru+1) + '.txt', 'a')
        # keep the permutation value
        arq_sp_p = open('res_spec/Cb_spec_p_' + str(ru+1) + '.txt', 'a')
        # keep the permutation value pos constructive phase
        arq_sp_con_p = open('res_spec/Cb_spec_const_p_' + str(ru+1) + '.txt', 'a')
        # keep grasp iterations
        arq_sp_iter = open('res_spec/Cb_spec_iter_' + str(ru+1) + '.txt', 'a')
        ...
        for k in k_all:
            ...
            for ide, (graph, nel, nl) in enumerate(all_graphs):
                ...
                if t_k < k:
                    arq_sp.write(str(ide) + str(',') + str(k) + str(',') + str(-1) + str(',') + str(-1) + str(',') +
                                 str(-1) + '\n')
                    arq_sp_p.write(str(ide) + str(',') + str(k) + str(',') + str(-1) + '\n')
                    arq_sp_con_p.write(str(ide) + str(',') + str(k) + str(',') + str(-1) + '\n')
                    ...
                    for ite in np.arange(it_ma):
                        ...
                        arq_sp_iter.write(str(ide) + str(',') + str(k) + str(',') + str(round(alpha, 2)) + str(',') +
                    ...
                    arq_sp.write(
                        str(ide) + str(',') + str(k) + str(',') + str(best_cost_con) + str(',') + str(best_cost) +
                        str(',') + str(timedelta(seconds=end_time - start_time)) + '\n')
                    arq_sp_p.write(str(ide) + str(',') + str(k) + str(',') + str(best_solution) + '\n')
                    arq_sp_con_p.write(str(ide) + str(',') + str(k) + str(',') + str(best_solution_con) + '\n')
