"""
Este código é uma implementação feita por Bruna Cristina Braga Charytitsch do método GRASP_N2V: Cb_N2V + Local Search.
O método para construção de uma solução inicial G_N2V utilizando a técnica node2Vec para geração de embeddings foi
...
This code is an implementation by Bruna Cristina Braga Charytitsch of the GRASP_N2V: Cb_N2V + Local Search method.
The method for constructing an initial solution G_N2V using the node2Vec technique for generating embeddings was
...
def pre_emb_data(graph0, nl0, nel0):
    ...
    # default parameters proposed by CogDL
    model = Node2vec(128, 80, 40, 5, 10, 10, 1, 1)
        ...
        for j in np.arange(cl1.shape[1]):
            ...
            if no_in != -1:
                ...
                if i == 0:
                    ...
                    # No neighbors not present or not inserted
...
def main():
    ...
    for ru in np.arange(total_run):
        ...
        arq_cb = open('res_n2v/Cb_n2v_' + str(ru + 1) + '.txt', 'a')
        # keep the permutation value
        arq_cb_p = open('res_n2v/Cb_n2v_p_' + str(ru + 1) + '.txt', 'a')
        # keep the permutation value pos constructive phase
        arq_cb_con_p = open('res_n2v/Cb_n2v_const_p_' + str(ru + 1) + '.txt', 'a')
        # keep grasp iterations
        arq_cb_iter = open('res_n2v/Cb_n2v_iter_' + str(ru + 1) + '.txt', 'a')
        ...
        for k in k_all:
            ...
            for ind_, (graph, nel, nl) in enumerate(all_graphs):
                ...
                if t_k < k:
                    arq_cb.write(str(ind_) + str(',') + str(k) + str(',') + str(-1) + str(',') + str(-1) + str(',')
                                 + str(-1) + '\n')
                    arq_cb_p.write(str(ind_) + str(',') + str(k) + str(',') + str(-1) + '\n')
                    arq_cb_con_p.write(str(ind_) + str(',') + str(k) + str(',') + str(-1) + '\n')
                    ...
                    # UM EMBEDDING POR GRAFO / ONE EMBEDDING FOR EACH GRAPH
                    ...
                    for ite in np.arange(it_ma):
                        ...
                        arq_cb_iter.write(str(ind_) + str(',') + str(k) + str(',') + str(round(alpha, 2)) + str(',') +
                    ...
                    arq_cb_p.write(str(ind_) + str(',') + str(k) + str(',') + str(best_solution) + '\n')
                    arq_cb_con_p.write(str(ind_) + str(',') + str(k) + str(',') + str(best_solution_con) + '\n')
