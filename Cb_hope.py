"""
Este código é uma implementação feita por Bruna Cristina Braga Charytitsch do método GRASP_HOPE: Cb_hope + Local Search.
O método para construção de uma solução inicial G_HOPE utilizando a técnica HOPE para geração de embeddings foi
...
This code is an implementation by Bruna Cristina Braga Charytitsch of the GRASP_HOPE: Cb_hope + Local Search method.
The method for constructing an initial solution G_HOPE using the HOPE technique for generating embeddings was
...
def pre_emb_data(graph0, nl0, nel0):
    ...
    # dimension = 2*number of nodes - 1
    # specify the dimension as two times the number of nodes minus one (dimension = 2* number of nodes - 1),
    # this will ensure that the value of k is smaller than the dimensions of matrix A. The error occurs when k is
    # greater than or equal to the number of columns or rows of the matrix, so setting k in this way should help avoid
    # this problem (because de SVD decomposition).

    # HOPE(dimension, beta)
    model = HOPE(2 * g_cogdl.num_nodes - 1, 0.01)
...
def main():
    ...
    for ru in np.arange(total_run):
        ...
        arq_hope = open('res_hope/Cb_Hope_' + str(ru + 1) + '.txt', 'a')
        # keep the permutation value
        arq_hope_p = open('res_hope/Cb_Hope_p_' + str(ru + 1) + '.txt', 'a')
        # keep the permutation value pos constructive phase
        arq_hope_con_p = open('res_hope/Cb_Hope_const_p_' + str(ru + 1) + '.txt', 'a')
        # keep grasp iterations
        arq_hope_iter = open('res_hope/Cb_Hope_iter_' + str(ru + 1) + '.txt', 'a')
        ...
        for k in k_all:
            ...
            for ide, (graph, nel, nl) in enumerate(all_graphs):
                ...
                if t_k < k:
                    arq_hope.write(str(ide) + str(',') + str(k) + str(',') + str(-1) + str(',') + str(-1) + str(',') +
                                   str(-1) + '\n')
                    arq_hope_p.write(str(ide) + str(',') + str(k) + str(',') + str(-1) + '\n')
                    arq_hope_con_p.write(str(ide) + str(',') + str(k) + str(',') + str(-1) + '\n')
                    ...
                    for ite in np.arange(it_ma):
                        ...
                        arq_hope_iter.write(str(ide) + str(',') + str(k) + str(',') + str(round(alpha, 2)) + str(',') +
                    ...
                    arq_hope_p.write(str(ide) + str(',') + str(k) + str(',') + str(best_solution) + '\n')
                    arq_hope_con_p.write(str(ide) + str(',') + str(k) + str(',') + str(best_solution_con) + '\n')
